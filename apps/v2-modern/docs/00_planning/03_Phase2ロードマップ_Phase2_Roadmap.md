...
| **2.2** | 会話・データ安定化（テキスト主導） | **[一部保留]** リファクタリング<br>テキスト入力主導 + 音声補助<br>~~タグ選択UI~~（保留）<br>**データモデル optional 化（O-02 実装済）**<br>JSONバリデーション/リトライ/回復<br>~~待機フィラー~~（保留） | **一部完了**<br>O-02 実装済み（Partial型対応）<br>F-01/04/07 は保留 | R2, R4 |
