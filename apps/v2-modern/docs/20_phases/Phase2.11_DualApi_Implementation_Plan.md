...
作成者: Codex+GPT-5  
更新日: 2026-02-11
更新日: 2026-02-25（現行実装に合わせて計画内容を実装反映版へ更新）
...
- 現行実装では、クライアント入力で `provider` は受け取らず、Workers 環境変数 `AI_PROVIDER` で切替します。
...
1. 現状固定と切替方針の確立（推定: 30-60分）
   - `AI_PROVIDER` 未指定時は `openai` を使う既定値を定義。
2. WorkersのProvider抽象化（推定: 90-150分）
   - 既存 OpenAI 呼び出しを provider 指定可能な共通実装へ整理。
   - Gemini用エンドポイント/キー/モデルの分岐を追加。
3. Gemini接続（OpenAI互換）とレスポンス正規化（推定: 60-90分）
...
5. テスト拡充（推定: 90-150分）
   - integration: `AI_PROVIDER=openai/gemini` 分岐、429フォールバック条件、`GEMINI_*` 設定反映を検証。
   - e2e: 実費シナリオで provider 別の応答時間とエラー率を収集。
6. iPhone実機検証と記録（推定: 30-60分）
   - OpenAI/Gemini 各3シナリオで応答時間・エラー率・体感コメントを収集。
   - 結果を `docs/50_reviews` 配下へ記録。

## 4. 受け入れ基準（DoD）

- `AI_PROVIDER` 未指定時に既存挙動（OpenAI）が完全維持される。
- `AI_PROVIDER=gemini` で `reply` / `extracted` のスキーマが通る。
- 既存テスト + 追加テストが全て通る。
...
- `AI_PROVIDER=openai` へ戻すことで即時に OpenAI 固定へ復帰可能にする。
- 問題発生時は `ENABLE_PROVIDER_FALLBACK` の有効化/無効化を含め、Workers環境変数のみで切り戻せるようにする。
