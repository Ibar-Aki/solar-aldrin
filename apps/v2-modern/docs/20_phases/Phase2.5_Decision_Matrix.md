...
| :--- | :--- | :--- | :---: | :--- | :--- | :--- |
| **OPS-01** | SSEストリーミング | 中 | A | ✖ 不採用 | ✖ 未実装 | **代替済**: Skeleton UI実装 |
| **OPS-02** | 天候APIキャッシュ | 低 | B | ✖ 不採用 | ✖ 未実装 | 自己申告で十分 |
| **OPS-03** | Sentry導入 | 低～中 | A | ✔ 採用 | ✔ **実装済** | main.tsx, workers/index.tsで初期化 |
| **OPS-04** | 構造化ログ | 低 | B | ✔ 採用 | ☔ 一部実装 | **PIIマスキング必須** |
| **OPS-05** | KPI収集 | 中 | B | ✔ 採用 | ☔ テストのみ | 実費テストで一部実施済 |
| **OPS-06** | Web Vitals | 低 | B | ✔ 採用 | ✔ **実装済** | main.tsxで送信開始 |
| **OPS-07** | AI信頼性監視 | 極低 | A | ✔ 採用 | ☔ 一部実装 | Zodエラーログ済 |

---

## 📝 今後のアクション

- **OPS-03 (Sentry)**: ✔ 実装済。本番公開済み。
- **OPS-06 (Web Vitals)**: ✔ 実装済。main.tsxで送信開始。
- **OPS-07 (AI Reliability)**: 一部実装済（Zodエラーログ）。
- **OPS-04**: PIIマスキング設計を行った上で導入する。
- **OPS-01**: ストリーミング不採用の代償として、Skeleton UI 実装済。
