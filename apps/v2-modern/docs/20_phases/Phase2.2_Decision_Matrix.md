...
| **コスト** | **低**（AI生成ではなく、固定の「はい/いいえ/終わる」等の定型ボタンのみ実装） |
| **推奨案** | ✅ **採用**（低コスト範囲で実装） |
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **UX-01** | **フィラー演出** | 低 | 採用 | ✅ **採用** | ⚠️ **UIのみ実装（保留）** | 音声未実装 |
| **UX-02** | **手袋モード** | 中 | 採用 | ❌ 不採用 | ❌ **未実装** | |
| **UX-03** | **ハイブリッド入力** | **低** | 採用 | ✅ **採用** | ⏸ **保留** | 固定チップのみ |
| **UX-04** | **タグ選択UI** | 低 | 採用 | ✅ **採用** | ⏸ **保留** | |
| **UX-05** | **完了演出** | 低 | 採用 | ✅ **採用** | ✅ **実装済** | ファンファーレ追加要 |
| **UX-06** | **PDF出力** | 中 | 採用 | ✅ **採用** | ✅ **実装済** | |
| **UX-07** | **音声認識** | 中 | 保留 | ⏸ 保留 | ✅ **実装済** | 補助機能として |
