...
更新日: 2026-02-18（会話選択UIを参考情報帯へ一本化し、進捗ラベル「KY活動→安全確認→総括」を撤去。完全音声会話の起動時は初回AI音声完了後にマイク自動開始するゲートを追加。再開時は先頭でAIガイド音声を再生）
更新日: 2026-02-18（完全音声会話の安定化: 起動失敗時の自動再試行を追加。再開時はメッセージ0件でも先頭ガイド音声を再生。SpeechRecognition の旧インスタンス onend 競合を防止）
更新日: 2026-02-19（完全音声会話の安定化を追加: full_voiceで中間認識結果を即時反映、fatal後の手動開始でautoRestartを再有効化、初期ブート中のChatBubble自動読み上げを抑止、TTSのonstart前onerror・isSpeaking固着・not-allowed再試行（次ユーザー操作）を実装）
