...
更新日: 2026-02-25（AI実装戦略の現況、プロバイダ構成の記述を現行コードへ整合）
...
**※ 詳細なER図やAPI設計については、[06_System_Design.md](06_System_Design.md) を参照してください。**
...
* **APIコスト**: 既定は OpenAI `gpt-4o-mini`（`AI_PROVIDER=gemini` 時は `gemini-2.5-flash`）。構成次第で1セッションあたりの単価を調整可能。
...
* **サーバー側構造化出力 (Current)**: `/api/chat` はサーバー側でJSON整形・スキーマ検証・回復処理（parse retry）を行い、クライアントは検証済みレスポンスを利用。
* **レスポンス形式 (Current)**: OpenAIは `json_schema`、Geminiは `json_object` を利用し、サーバー側で同一形式へ正規化。
* **RAG (将来拡張)**: 過去の災害事例検索など、独自データを注入しやすい構成(Vector Store連携)を見据えたAPI設計。

### 3. デファクトスタンダードの採用（業界標準への準拠）
...
| 技術 | 選定理由 |
| :--- | :--- |
| **OpenAI / Gemini（切替対応）** | `AI_PROVIDER`、`OPENAI_MODEL`、`GEMINI_MODEL` で切替可能。可用性・コスト・応答品質を運用で最適化できる。 |
...
* **トリガー (Trigger)**: ユーザー送信完了から **350ms** 後
* **アクション (Action)**: 音声合成で「了解。」と発話 (Rate: 1.3〜1.5相当)
* **条件 (Condition)**:
...
* **技術 (Technology)**: **Groq API** (LPU推論エンジン)
* **モデル (Model)**: `distil-whisper-large-v3-en` (または日本語最適化モデル)
* **性能 (Performance)**:
  * **遅延 (Latency)**: < 500ms (リアルタイム係数 ~100x)
  * **コスト (Cost)**: 約 0.11ドル / 時間 (OpenAIの約1/3)
* **アーキテクチャ (Architecture)**:
  * クライアント: 音声をBlobとして録音
  * エッジ (Workers): Groq APIへプロキシ送信
  * レスポンス (Response): テキストを即時返却
* **メリット (Advantage)**: 建機ノイズ、方言、専門用語に対する圧倒的な認識耐性を、ブラウザ標準並みの速度で提供可能。
