...
- **4Rフロー**: 危険発見→要因分析→対策決定→レビューの流れを実装
- **テンプレローテ**: 質問の切り口を作業ごとに変化（DLG-04実装済）
- **Persona**: 「**KYパートナー**」ペルソナ（※「新人記録係」は保留）
- **保留**: 5W1H深掘りフロー、選択式深掘りUI、振り返り入力
- **Observability**: Sentry統合、Web Vitals、カスタムテレメトリ収集
- **不採用**: SSEストリーミング（代替: Skeleton UI）、天候APIキャッシュ
...
- **Latency**: Skeleton UIにより体感待ち時間を改善
