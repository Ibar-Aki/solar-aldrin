            ...
            migrate: (persisted, version) => {
                ...
                if (session) {
                    session.workItems = session.workItems.map((item) => ({
                        ...item,
                        countermeasures: normalizeCountermeasuresInput((item as { countermeasures?: unknown }).countermeasures),
                    }))
                }

                state.currentWorkItem = {
                    ...state.currentWorkItem,
                    countermeasures: normalizeCountermeasuresInput((state.currentWorkItem as { countermeasures?: unknown }).countermeasures),
                }
