...
- 依存パッケージの脆弱性が確認できた場合、直接依存は原則として同一作業内で安全版へ更新し、更新可否を検証します。
- Node系プロジェクトでは、少なくとも production 依存に対する `npm audit --omit=dev` 相当の監査を CI で維持します。
