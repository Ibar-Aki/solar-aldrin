...
### 高
2) 認証なし + CORS `*` で全API公開  
   - 位置: `src/workers/index.js`（CORS設定と /api/* ルーティング）  
   - 影響: 第三者がAPIを自由に呼び出せる。コスト増/悪用リスク。  
   - 推奨: 最低限の認証（固定トークン/CF Access 等）とOrigin制限を導入。

3) 記録APIが無認証でデータ保存/取得  
   - 位置: `src/workers/index.js`（/api/records の POST/GET）  
...
### 低
5) CDNスクリプトにSRIなし  
   - 位置: `src/public/qr.html`、`src/public/js/screens/done.js`  
   - 影響: 供給元改ざん時の影響を受けやすい。  
   - 推奨: SRI付与、またはローカル同梱。

6) エラー詳細の外部露出  
   - 位置: `src/workers/index.js`（例外の `error.message` を返却）  
   - 影響: 内部情報が外部へ露出する可能性。  
