...
| :--- | :--- | :--- | :--- | :--- |
| **2.1** | 運用防御とコスト制御 | レート制限/鍵管理/Origin強化<br>入力検証（長さ・禁止語・役割制限）<br>APIコスト監視/アラート | 公開APIの濫用を防げること<br>不正/暴走リクエストが遮断されること | R1, R2, R5 |
| **2.2** | 会話・データ安定化（テキスト主導） | テキスト入力主導 + 音声補助<br>タグ選択UI（よくある危険）<br>データモデルの optional 化<br>JSONバリデーション/リトライ/回復<br>待機フィラー/完了演出 | 必須項目欠落でもセッション完了可<br>JSON破損でも状態破壊しない | R2, R4 |
| **2.3** | コンテキスト注入/ナレッジ循環 | 過去ヒヤリハット/昨日指摘の参照<br>簡易RAG（履歴検索 + プロンプト注入）<br>天候/曜日の文脈強化 | 「毎回同じ助言」を抑制<br>過去の危険が再利用される | R3, R4 |
...
| :--- | :--- | :---: | :---: | :---: | :---: | :---: | :---: | :--- | :--- |
| API保護（レート制限/鍵管理） | R2, R5 | 4 | 5 | 3 | 2 | 2 | 16 | 2.1 | コスト暴騰/不正利用の最短封じ |
